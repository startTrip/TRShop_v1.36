...
final class CameraConfigurationManager {
    ...
    private static Point findBestPreviewSizeValue(
        ...
        for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			/*
             * zxing 作者的意图很明显,因为相机的预览像素不等于屏幕的显示像素,作者想找到最合适的.
			 * 在预览方向和屏幕方向一致的情况下,差值最小的就是最优的. 原代码int newDiff = Math.abs(newX -
			 * screenResolution.x) + Math.abs(newY - screenResolution.y);
			 */
            if (newDiff == 0) {
                bestX = newX;
                bestY = newY;
